(function( $ ) {
	$.fn.gobutton = function(userSettings) {
		...
		loaderImage = $('<svg width="88px" height="88px" viewBox="1310 528 101 101" version="1.1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" id="rotate"><desc>Created with Sketch.</desc><defs><linearGradient x1="69.3641269%" y1="96.8508376%" x2="60.2027223%" y2="81.443299%" id="gradient"><stop stop-color="#FFFFFF" offset="0%"></stop><stop stop-color="#FFFFFF" stop-opacity="0" offset="90%"></stop></linearGradient></defs><path d="M1368.7985,626.292822 C1391.63073,622.356234 1409,602.456522 1409,578.5 C1409,551.71419 1387.28581,530 1360.5,530 C1333.71419,530 1312,551.71419 1312,578.5 C1312,599.965964 1325.94552,618.174777 1345.27099,624.560849" id="oval" stroke="url(#gradient)" stroke-width="4" stroke-linecap="round" fill="none"></path></svg>');

		wrap = $('<div class="circleUngate gobutton"><div class="loader"></div></div>');
		...
		return this.each(function() {
			...
			this.classList.add('circleUngate');
			...
			gobutton.$content_loader[0].addEventListener('animationstart', function(event) {
				switch (event.animationName) {
					case 'spinUngate':
						...
						break;
					case 'stopspinUngate':
						this.classList.remove('spinUngate');
						break;
				}
			...
			gobutton.$content_loader[0].addEventListener('animationend', function(event) {
				...
				if (event.animationName === 'stopspinUngate' || event.animationName === 'stopUngate') {
					this.classList.remove('spinUngate');
		...
		function runAnimation() {
			if (!this.$content_loader[0].classList.contains('spinUngate')) {
		...
		function animationStart() {
			this.$content_loader[0].classList.add('spinUngate');
			this.$content_loader[0].classList.remove('stopspinUngate');
		...
		function animationInfiniteStart(speed) {
			this.$content_loader[0].classList.add('spinUngate');
			this.$content_loader[0].classList.remove('stopspinUngate');
			this.$content_loader[0].style.animation = 'infinitespinUngate ' + (speed || 0.12) + 's linear 0s infinite';
		}

		function animationStop() {
			this.$content_loader[0].classList.add('stopspinUngate');
			this.$content_loader[0].classList.remove('spinUngate');
		...
		function returnAnimation() {
			...
			var animations = {
				...
				infinite: ' infinitespinUngate ' + infinitespinSpeed + 's linear ' + animationSpeed + 's infinite'
			};
